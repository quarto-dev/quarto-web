...
Toolbars are similar to sidebars, but provide a horizontal layout for inputs. Create a toolbar by adding the `.toolbar` class to a row heading. For example, here we create a toolbar for an interactive dashboard that uses Shiny:
