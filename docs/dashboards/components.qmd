...
All dashboards include a top-level navigation bar that can provides a logo, title, and author. Dashboards with multiple pages also contain a link to each page on the navigation bar. 

<!--- TODO: Screenshot of navigation bar with pages --->
...
As illustrated above, links to each of the dashboard's pages are provided automatically. 

## Pages

Dashboards consist of one or more pages. By default, dashboards have only a single page. To introduce multiple pages, use level 1 headings above the level 2 headings used to define rows and columns. For example:
...
Within a page, dashboards components are laid out using alternating sets of rows and columns. Rows and columns are in turn defined by markdown headings and computational cells. For example, here's a simple layout with two rows, where the second row is split into two columns:
...
Here, level 2 markdown headings (e.g. `## Row {height=70%}`) define the contents of rows as well as their relative height. The ```` ```{python} ```` code cells in turn automatically create cards that are laid out horizontally within the row.

::: {.callout-note}
While all of the layout examples below will use a single-page dashboard, they work just the same when dashboards include multiple pages.
:::
...
<!--- TODO: scrolling: true once it comes online --->
