...
While you often fill dashboard cards with plots and tables, you can also include arbitrary markdown content anywhere within a dashboard. For example, here is a dashboard where the last card in a column is plain markdown:
...
Note that if you are authoring using a Jupyter Notebook then markdown cells automatically become `.content` divs (i.e. they don't need the explicit `:::` div enclosure).

To include `.content` within an adjacent card rather than give it its own card, use the `within` attribute (specifying `"next-card"` or `"previous-card"`). For example:
