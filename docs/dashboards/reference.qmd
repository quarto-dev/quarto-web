...
Create [Sidebars](components.qmd#sidebars) by applying the `.sidebar` attribute to a level 1 heading (for global sidebars) or level 2 heading (for page level sidebars)
...
| Class     | Description                                                          |
|--------------------|----------------------------------------------------|
| `.tabset` | Contents should be arranged into a [Tabset](components.qmd#tabsets). |
...
These same options can be applied to `.content` divs. For example:

``` {.python .pymd}
::: {.content title="Life Expectancy" padding="3px"}
This is the content.
:::
```
...
Note that value box options can be specified either as cell options or by printing a `dict()` (for Python) or `list()` (for R) (this is helpful when options need to be dynamic). See the [Value Boxes](data-presentation.qmd#value-boxes) component documentation for details.