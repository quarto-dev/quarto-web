...
- [Card Inputs](#card-inputs) provide a way to layout card-specific inputs.
...
Sidebars are a great place to group inputs for interactive dashboards. To include a sidebar, add the `.sidebar` class to a level 2 heading. For example:
...
Sidebars can be located on either the left or right side. Layout a sidebar on the right by including it _after_ the column(s) it is adjacent to. You can also modify the size of sidebars using the `width` attribute. This example demonstrates both of these techniques:
...
Toolbars are similar to sidebars, but provide a horizontal layout for inputs. Create a toolbar by adding the `.toolbar` class to a level 2 row heading. For example:
...
Toolbars can be located on either the top or bottom of content. Layout a toolbar on the bottom by including it _after_ the rows(s) it is adjacent to. For example:
...
In some cases you may want to marry inputs more directly to a single output. You can do this using either a card toolbar or card sidebar.
...
To add a sidebar to a card, define it immediately to the left or the right of the cell that generates the output. YOu can do this either by adding the `content: card-sidebar` option to a call or by creating a div with the `.card-sidebars` class. For example:
