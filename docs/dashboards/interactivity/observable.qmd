...
```{{ojs}}  # <1>
data = FileAttachment("palmer-penguins.csv") # <1>
  .csv({ typed: true }) 
```  # <1>

```{{ojs}} # <2>
filtered = data.filter(function(penguin) {  # <2>
  return bill_length_min < penguin.bill_length_mm && # <2>
         islands.includes(penguin.island); # <2>
}) # <2>
```
...
1.  We read the raw penguins dataset from a CSV when the page loads.

2.  `filtered` is a value that is automatically recomputed when variables declared with `viewof` change (in this case `bill_length_min` and `islands`).

3.  Create sidebars by adding the `.sidebar` class to a level 2 header. Sidebars can include code cells as well as images, narrative, and links.

4.  Here we define our inputs using `viewof` so that the `filtered` dataset is automatically recomputed when they change.

5. Level 1 headings (here `# Plots` and `# Data`) create pages within the dashboard.

6.  The plot is automatically redrawn whenever the `filtered` dataset changes.

7.  The tabular data display is automatically refreshed whenever the `filtered` dataset changes.
