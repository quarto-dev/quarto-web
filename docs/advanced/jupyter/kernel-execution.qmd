...
- `daemonize`: if a kernel returns `true` when executing Quarto's setup cell, then Quarto will maintain a persistent kernel. The daemon options are the same as in the [documentation above]([Python](https://quarto.org/docs/computations/python.html#kernel-daemon)).
