...
- `quarto_setup_cell`: the source code for a setup cell to be executed by nbclient at the beginning of execution of any Quarto notebook or document. Quarto will create a Jupyter notebook cell with the contents of this file as the `source` field. In addition, Quarto will set Jupyter cell metadata with the configuration options as described above, under the compound key `quarto`/`setup`.
- `quarto_cleanup_cell`: analogously to above, the source code for a cleanup cell. Metadata is also provided. Note that for the cleanup cell execution, the metadata will appear under the compound key `quarto`/`cleanup` instead.
...
Arbitrary Jupyter kernels can indicate support for daemons. To do so, provide a `quarto_setup_cell` file in the kernelspec directory so that setup cells can be executed, and ensure that the execution of the setup cell returns metadata indicating support for running as a daemon. The supported metadata options are all under the key `quarto`:

- `quarto`/`daemonize`: if a kernel returns `true`, then Quarto will maintain a persistent kernel. The daemon options are the same as in the [documentation above]([Python](https://quarto.org/docs/computations/python.html#kernel-daemon)).
- `quarto`/`restart_kernel`: if a kernel returns `true`, then Quarto will forcibly restart the kernel. This allows kernels to detect that their configuration has been changed such that it cannot support continued execution in the same process.