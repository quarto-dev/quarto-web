...
Not all of the metadata keys are used in every format. For example, the HTML format does not display the `keywords`. Currently the JATS format makes use of the broadest set of metadata tags, so if you want to check how things render we recommend previewing with `format: jats`.
