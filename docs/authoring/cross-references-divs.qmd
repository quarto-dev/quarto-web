---
title: Div Syntax for Cross References
---
...
Note that the above example is a non-executable code cell. To cross-reference code in an executable code cell you need to use code cell options, not the div syntax, as described in [Cross-references for executable code blocks]().
...
If you would rather give diagrams a label and counter distinct from figures, consider defining a [custom cross-reference]() type.
...
If you would rather give videos a label and counter distinct from figures, consider defining a [custom cross-reference]() type.
