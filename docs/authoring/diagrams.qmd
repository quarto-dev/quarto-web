...
You might find it more convenient to edit your diagram in a standalone file and then reference it from within your `.qmd` document. You can do this by adding the `file` option to a Mermaid or Graphviz cell.  For example:
...
Note that the cell options syntax is slightly different for `{mermaid}` and `{dot}` because they have distinct character sequences for single-line comments.
