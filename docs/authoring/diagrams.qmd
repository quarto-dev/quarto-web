...
4)  The [Quarto VS Code Extension](https://marketplace.visualstudio.com/items?itemName=quarto.quarto) (also bundled with Positron from [Open VSX](https://open-vsx.org/extension/sumneko/lua)) supports live preview of diagrams embedded in `.qmd` files and in `.mmd` and `.dot` files:
