...
Note that the cell options syntax is slightly different for `{mermaid}` and `{dot}` (`%%|` and `//|`, respectively) because they have distinct character sequences for single-line comments. Cell options **must** be included directly beneath the start of the diagram code chunk.
