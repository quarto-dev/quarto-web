...
Examples above are using the shorter form when you only set a language for highlighting. Longer form using language as class (i.e. `.python`) inside brackets can also be used and allows to add attributes to the block of source code (like with Divs - see @sec-divs-and-spans). Some specific features are using this syntax like [Lines Numbers](/docs/output-formats/html-code.qmd#line-numbers) or [Code Filename](/docs/output-formats/html-code.qmd#code-filename). Here is an example for the latter:
