...
Use raw blocks to include content directly without Quarto parsing it. A raw block starts with ` ```{=` followed by a format and closing `}`, e.g. here's a raw HTML block:
...
You can also include raw syntax inline with a raw span:
```markdown
