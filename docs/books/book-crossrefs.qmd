...
To suppress cross-reference prefix, use bracket syntax and prepend `-` before `@`, like so `[-@citation]`. This will suppress default prefix, e.g. `[-@sec-visualization]` will produce just `1.1` (or whatever number this section happens to be) without default prefix.

``` markdown
See [-@sec-visualization].
```

To refer to a chapter or appendix explicitly you should spell out "Chapter" or "Appendix" as prefix in the bracket syntax for cross reference:
