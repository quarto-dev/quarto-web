...
```{.yaml example-link="examples/footer-and-logo.qmd"}
---
title: "Presentation"
format:
  revealjs:
...
```{.markdown example-link="examples/per-slide-footer.qmd"}
## Slide Title
...
````{.java example-link="examples/line-highlighting-1.qmd"}
...
![](images/line-highlight-discontinuous.png)
...
````{.java example-link="examples/line-highlighting-3.qmd"}
...
<!--- FIXME example-link="" doesn't work here... --->
...
```{.python example-link="examples/executable-code-figure-size.qmd"}
...
```{.markdown example-link="examples/tabset.qmd"}
::: {.panel-tabset}
...
```{.markdown example-link="examples/background-color.qmd"}
...
```{.markdown example-link="examples/image-background.qmd"}
