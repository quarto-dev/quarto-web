...
## Known limitation with scrollable slides

If `scollable: true` (or `.scrollable` on a slide) is set at the same time, the image size calculation will not work well. In such situations, some configurations may need to be adjusted depending on the output:

- Apply manual stretching using `auto-stretch: false` and then `.r-stretch` on image slide.
- Opt-out with `.nostretch` on slides with `.scrollable`
