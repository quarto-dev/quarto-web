...
The basic workflow for editing notebooks for use with Quarto is identical to the workfow for `.qmd` files. Edit the file and then execute the **Quarto: Render** command (you can alternatively use the <kbd>Ctrl+Shift+K</kbd> keyboard shortcut). You'll see a preview of the published page side by side with the notebook:
