...
Note also that you can access all editing commands via [keyboard shortcuts](options.qmd#shortcuts).
...
Currently, the [gfm](../output-formats/gfm.qmd) and [hugo](../output-formats/hugo.qmd) (with `enableEmjoi = true` in the site config) formats both support text representation of emojis.
...
When reviewing a document you often want to provide inline comments with suggested revisions.
...
The best way to include CSS styles is to create an external stylesheet (e.g. `styles.css`), then include it within the output options of your document.
For example:
...
Spans are special entities that allow you to apply identifiers and/or styles to a region of text.
