...
![](images/visual-editing-table-context.png){fig-alt="A right-click context menu on a table, with options to insert or delete rows and columns, delete the whole table, align a column, and toggle the table header and caption."}
...
|------------------------|-------------------------|------------------------|
| ![](images/visual-editing-list-item.png){fig-alt="A bulleted list with 'Item 1' and 'Item 2', and an empty third bullet with the cursor in it."} | ![](images/visual-editing-list-sublist.png){fig-alt="The same list, with the cursor now in an indented sub-bullet under 'Item 2'."} | ![](images/visual-editing-list-block.png){fig-alt="The same list, with the cursor now on a new line under 'Item 2' with no bullet."} |
...
Visual mode creates normal lists by default (you can change this behavior via [Editor Options](options.qmd)). You can toggle between normal and tight lists using the {{< kbd mac=Cmd-Alt-9 win=Ctrl-Alt-9 linux=Ctrl-Alt-9 >}} keyboard shortcut. You can also change the list type using the **Format -\> Edit Attributes** dialog (also accessible via the {{< kbd F4 >}} shortcut). If you have existing tight lists in your markdown source files, they will remain so within the visual editor.
...
![](images/visual-editing-attributes.png){fig-alt="A heading with '#er-injuries' and '.case-study' attribute pills shown at the top right, next to an edit button."}

Note that any ID as well as the first class specified within the attributes are also displayed. Click the edit button or use the {{< kbd F4 >}} keyboard shortcut to edit the attributes.

## Special Characters

### Hard Line Breaks

You can insert a [hard line break](https://practicaltypography.com/hard-line-breaks.html) using the **Insert -\> Special Characters -\> Hard Line Break** command or via the {{< kbd Shift-Enter >}} keyboard shortcut.

### Non-Breaking Spaces

You can insert a [non-breaking space](https://en.wikipedia.org/wiki/Non-breaking_space) using the **Insert -\> Special Characters -\> Non-Breaking Space** command or via the {{< kbd Ctrl-Space >}} keyboard shortcut. Non-breaking spaces are displayed with an alternate background color to distinguish them from normal spaces.
...
|------------------------------------|------------------------------------|
| ![](images/visual-editing-emoji-dialog.png){fig-alt="The emoji picker, with a search box, a grid of emoji, and the name and shortcode of the selected emoji shown at the bottom."}    | ![](images/visual-editing-emoji-completion.png){fig-alt="An autocomplete menu triggered by typing ':gr', listing matching emoji shortcodes such as ':grinning:' and ':grin:', each with its icon."} |
...
![](images/visual-editing-unicode.png){fig-alt="The Unicode character picker, with a search box, a grid of symbols, and the name and code point of the selected character shown at the bottom."}
...
![](images/visual-editing-spelling.png){fig-alt="The misspelled word 'resherch' underlined in red, with a right-click menu offering spelling suggestions, plus options to ignore the word or add it to the dictionary."}
...
![](images/visual-editing-comment.png){fig-alt="Two HTML comments highlighted with a peach background: one after 'Consumer Product Safety Commission' suggesting a link, and one marking a spot mid-sentence."}
...
To add a CSS class to a heading, use the edit button at the top right of the heading that appears when your cursor is within it (or use the {{< kbd F4 >}} keyboard shortcut):

![](images/visual-editing-css-heading.png){fig-alt="The 'Edit Attributes' dialog open over a heading, with empty ID and Classes fields; a nearby heading already shows an '.important' class pill."}
...
You can apply CSS classes to images (for example, to give them a special border) using the standard **Image** dialog. To do this, double-click the image (or use the {{< kbd F4 >}} shortcut with the image selected) and apply the desired classes:

![](images/visual-editing-css-image.png){fig-alt="The 'Figure' dialog's Attributes tab open over a selected image, with empty ID and Classes fields for applying a CSS class."}
...
You can create divs using the **Insert -\> Div** command, and you can apply attributes (including CSS classes) to divs the same way as you apply attributes to headings (clicking the edit button or using the {{< kbd F4 >}} keyboard shortcut when you are within a div):

![](images/visual-editing-css-div.png){fig-alt="The 'Div Attributes' dialog open over a div, with '.important' entered in the Classes field."}
...
![](images/visual-editing-css-span.png){fig-alt="The 'Span Attributes' dialog open over a span of text, with '.important' entered in the Classes field."}
