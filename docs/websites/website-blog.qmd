---
title: Quarto Blogs
description: "Create a blog by authoring a collection of posts and then dedicating a page (usually the website homepage) to a listing of all of your posts."
---

## Overview

Quarto websites include integrated support for blogging. Blogs consist of a collection of posts along with a navigational page that lists them in reverse chronological order.
...
To create a new blog, use the `quarto create-project` command. For example, to create a blog within the directory `myblog` you would do this:
...
::: callout-note
If you are using RStudio, you can alternatively use the **New Project** command to create a Quarto Blog. Note that this feature is only available in the [preview release](the preview release of RStudio, you can also) of RStudio.
:::
...
When you render a new post, the listing page will automatically updated, adding the most recent post to the top of the list. See the article on [Listing Pages](website-listings.qmd) to learn more about customizing listings, including use a grid layout rather than the default shown above.
...
1.  In the `quarto.yml` file, add a `site-url` and `description` to the `website` key (without these options being set in the project file, Quarto cannot generated a feed). For example:
