...
When you click a search result, Quarto navigates to the destination page and highlights the matching terms with yellow `<mark>` elements. This works with both the built-in search and [Algolia](#using-algolia) search.
...
URLs that combine a search query with a section anchor (e.g., `page.html?q=term#section`) scroll to the specified section rather than jumping to the first match.

The screenshot below shows a search for `"## Heading 2\n"` with a `#raw-output` anchor: the Knitr tab is activated (instead of the default Jupyter tab), matches are highlighted inside the code block, and the page scrolls to the Raw Output section:

![Search highlighting activating the Knitr tab and highlighting matches inside a code block on the Quarto prerelease site.](images/search-highlight-tabset.png){.border .light-content fig-alt="Quarto website showing search result highlighting: the Knitr tab is activated inside a tabset, yellow highlighted marks appear on code lines containing cat statements, and the page is scrolled to the Raw Output section."}

![Search highlighting activating the Knitr tab and highlighting matches inside a code block on the Quarto prerelease site, dark mode.](images/search-highlight-tabset-dark.png){.border .dark-content fig-alt="Quarto website in dark mode showing search result highlighting: the Knitr tab is activated inside a tabset, yellow highlighted marks appear on code lines containing cat statements, and the page is scrolled to the Raw Output section."}
