...
Quarto processes EJS templates as Markdown, so we reccomend using the `.ejs.md` extension for your template files. Since most templates output HTML, wrap your HTML in a raw block (`` ```{=html} ``) to avoid performance issues and escaping problems.
