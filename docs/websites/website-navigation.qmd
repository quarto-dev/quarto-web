...
For more information on controlling the appearance of the side navigation using HTML themes, see [HTML Themes - Navigation](/docs/output-formats/html-themes.qmd#navigation).
...
The sidebar auto generation feature described below is currently only available in the Quarto v1.2 pre-release: <https://quarto.org/docs/download/prerelease.html>.
:::
...
Using `auto: true` at the root level will result in all documents in your website being included within the navigation (save for the home page which can be navigated to via the title link). Navigation is constructed using the following rules:

1. Navigation item titles will be read from the `title` field of documents.

2. Subdirectories will create sections and will be automatically titled based on the directory name (including adding capitalization and substituting spaces for dashes and underscores). Use an `index.qmd` in the directory to provide an explicit `title` if you don't like the automatic one.

3. Order is alphabetical unless a numeric `order` field is provided in document metadata.

Rather than specifying that all documents should be included, you can also specify a directory name or one or more glob patterns. For example, the following values for `auto` are all valid:

```yaml
auto: docs
auto: "*-report.ipynb"
auto: ["*.qmd", "*-report.ipynb"]
```

Note that `auto` entries that include a `*` need to have quotes around them so they are correctly parsed as YAML strings.

The `auto` option can also be used anywhere within a manually constructed sidebar `contents`. For example, here we add a section that is automatically generated from a directory:
...
Here we add an `auto` entry in the middle of a list of items:
