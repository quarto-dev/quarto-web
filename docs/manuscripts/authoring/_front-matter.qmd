...
## Expand to see full YAML header for `{{< meta article-file >}}`
...
For example, at the top level the header in `{{< meta article-file >}}` sets the following keys: `title`, `author`, `keywords`, `abstract`, `plain-language-summary`, `key-points`, `date`, `bibliography`, `citation`, and `number-sections`.
