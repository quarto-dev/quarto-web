...
The template manuscript includes some executable Python code. To run this code, you'll need the `matplotlib` and `numpy` packages:
...
To follow this tutorial you'll need your own copy of the [template repository]({{< meta tool.template-repo >}}).
...
2.  Once your repository is created, clone it to your local computer.
    ...
    ``` {.bash filename="Terminal"}
    git clone {{< meta tool.template-repo >}}.git
    ```
    ...
    ![](images/rstudio-open.png){fig-alt="Screenshot of the RStudio IDE with a project called manuscript-template-rstudio-test open. The File pane shows the folders: _extensions, _freeze, .github, images, and notebooks; and the files: _quarto.yml, .gitignore, index.ipynb, README.md and references.bib." .border}
...
3.  You'll be working inside this directory throughout the tutorial, so if you are ready to proceed, navigate inside the directory, and start Jupyter Lab:
    ...
    ![Template manuscript opened in Jupyter Lab](images/jupyter-open.png){fig-alt="A screenshot of Jupyter Lab. The File Browser is open showing the folders: _extensions, images, and notebooks; and the files: _quarto.yml, index.ipynb, README.md and references.bib." .border}
...
3.  You'll be working inside this directory throughout the tutorial, so if you are ready to proceed, open the directory in VS Code.
    ...
    ADD SCREENSHOT of VS Code open with files pane open showing directory structure.\
:::
