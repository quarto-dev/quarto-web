...
2.  Provide de-nomalized views authors together with affiliations such that it is straightforward for template authors to create the LaTeX required by Journals.

Below we'll explore these facilities in more detail both from the standpoint of both template authors and article writers.
...
The `author` metadata key receives a simple list of names that will render properly in most existing Pandoc templates note aware of the Quarto extended schema.
...
Quarto will parse names using BibTex (a la [openjournals/inara](https://github.com/openjournals/inara/blob/main/data/filters/normalize-author-names.lua)), supporting BibTexXs parsing behavior for comma count, capitalization, and so on. When the name is unparseable by BibTeX, Quarto will attempt to parse names into given and family using spaces (everything after the last space is considered a family name), but to disambiguate, you may provide the complete name including separate given, family ,and particle keys:
