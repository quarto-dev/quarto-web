...
Each Quarto document can be optionally processed by a computational engine (the engine can be manually specified or automatically detected based on the code chunks within). Current engines include Knitr (which is also used by R Markdown) and Jupyter (which provides support for Python, Julia, and other languages). See the documentation on [Engine Binding](https://quarto.org/docs/computations/execution-options.html#engine-binding) for additional details.
