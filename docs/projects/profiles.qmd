...
For example, this `_quarto.yml` file defines a `production` configuration that overrides the default rendering execution behavior (disabling the use of `freeze`):
...
When the `production` configuration is active, the YAML options it defines will be [merged](quarto-projects.qmd#metadata-merging) over the default project options (in this case resulting in the use of the frozen computational results being disabled).

The next section covers how to activate configuration profiles, then subsequent sections go into more depth on the various capabilities of profiles.

## Activating Profiles

One or more named configuration profiles can be activated as using either the `QUARTO_PROFILE` environment variable or the `--profile` command line argument.

Here we activate a configuration profile using the `QUARTO_PROFILE` environment variable:
...
Here we activate a configuration profile by passing the `--profile` argument on the Quarto command line:
...
Note that more than one configuration profile can be activated at once. For example, here we activate both the `production` and `advanced` profiles:
...
Above we demonstrated defining profile-specific project options. You do this by including named `configurations` within the `profile` key of `_quarto.yml`. Here is another example that customizes the title and output directory based on profile:
...
When authoring a Quarto project that uses configuration profiles you might want to change which profile(s) are rendered by default to try out different variations of the project.
...
Quarto includes a feature that allows for conditional inclusion of content based on the currently defined `lang` for the document. This is based on using the `lang` attribute on divs and spans. For example:
...
When rendered, this document will include only the English block (because `lang` is set to `en` in the document options). You can combine this feature with configuration profiles by setting the `lang` within your profile. For example:
