...
All Quarto projects include a `_quarto.yml` configuraiton file. Any document rendered within the project directory will automatically inherit the metadata defined at the project level. Here is an example of what the `_quarto.yml` file might look like:
...
You can further customize project metadata based on different configuration profiles (e.g. development vs. production or creating multiple versions of a book or website). See the article on [Configuration Profiles](profiles.qmd) for additional details.
...
-   [Configuration Profiles](profiles.qmd) describes how you can adapt both the options and content of your projects for different scenarios (e.g. development vs. production or creating multiple versions of a book or website).
