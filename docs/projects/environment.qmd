...
You may have one more more environment variables that you want to make sure are set whenever you render your project. You might use environment variables in one of these contexts:

1.  To provide the URL of a server to download data from.
2.  To set the address for a proxy server (e.g. `HTTP_PROXY`).
3.  To provide credentials for accessing a remote server (note that below we cover best practices for using credentials stored in environment variables).
4.  To tune the behavior of executable code (e.g. `OMP_NUM_THREADS`, `ARROW_IO_THREADS`, etc.)
5.  To provide options for language interpreters (e.g. `PYTHONINSPECT` or `TZDIR`).
6.  To determine which versions of Python and R are used (e.g. `QUARTO_PYTHON`, `QUARTO_R`, `PY_PYTHON`, or `RETICULATE_PYTHON`).

Quarto projects can define a set of standard environment variables as well a vary which environment variables are defined based on the [active project profile](profiles.qmd). It's also possible to use environment variables that are defined only on the local system and not checked in to version control.
...
One problem this leaves is ensuring that these variables are specified in local configurations. The next section on example environments covers how to do this.

## Example Environment

If you want to document and require that certain environment variables need to be specified, you can create an `_environment.example` file which can be copied to `_environment.local` as a starting point.

Return to the example of AWS credentials, this `_environment.example` file provides documentation that these credentials will be required to render the project locally:

``` {.ini filename="_environment.example"}
AWS_ACCESS_KEY_ID=
...
Note that any variable defined in the `_environment.example` is *required* and will result in an error message at startup if it is not defined in either the default or local environment.
