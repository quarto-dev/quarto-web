...
If your quarto document includes both `{python}` and `{r}` code blocks, then quarto will automatically use [reticulate](https://rstudio.github.io/reticulate/) to execute the python content and may use a different version of python than is included in your environment. To direct RStudio to use a specific version of python, you can specify a path to an installed python version by adding the following line to the `RProfile` file in your project directory:
