...
You can deploy Quarto sites to a Hugging Face space using the [Quarto template](https://huggingface.co/new-space?template=posit%2Fshiny-for-python-template). 
Simply create the space, clone it locally and then edit the quarto files in the `src` directory. 
You shouldn't need to modify the Dockerfile unless you need to add system dependencies.
