...
[Posit Connect Cloud](posit-connect-cloud.qmd) can publish directly from a GitHub repository, automatically re-rendering and redeploying your content when you push changes. This is the recommended approach for continuous deployment to Posit Connect Cloud.

See the Posit Connect Cloud documentation on [Publishing from GitHub](https://docs.posit.co/connect-cloud/user/publish/github.html) for setup instructions. For other CI environments, you can use `quarto publish` with environment variables as described in the [Shell Commands] section below.
...
| Posit Connect Cloud | `POSIT_CONNECT_CLOUD_ACCESS_TOKEN`, `POSIT_CONNECT_CLOUD_REFRESH_TOKEN`, and `POSIT_CONNECT_CLOUD_ACCOUNT_ID` |
...
```{.bash filename="Terminal"}
# render and publish to Posit Connect Cloud based on _publish.yml
