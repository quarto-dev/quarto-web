...
[Posit Connect Cloud](posit-connect-cloud.qmd) can publish directly from a GitHub repository, automatically re-rendering and redeploying your content when you push changes. This is the recommended approach for continuous deployment to Connect Cloud.

See the Connect Cloud documentation on [Publishing from GitHub](https://docs.posit.co/connect-cloud/user/publish/github.html) for setup instructions. For other CI environments, you can use `quarto publish` with environment variables as described in the [Shell Commands] section below.
