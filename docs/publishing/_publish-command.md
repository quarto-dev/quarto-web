...
quarto publish {{< meta provider.id >}}
```

If you haven't published to {{< meta provider.name >}} before, the publish command will prompt you to authenticate.

::: {.content-visible when-meta="provider.authorization.enabled"}

{{< meta provider.authorization.description >}}

```{.bash filename="Terminal"}
$ quarto publish {{< meta provider.id >}}
{{< meta provider.authorization.prompt >}}
```
...
```{.bash filename="Terminal"}
quarto publish {{< meta provider.id >}} document.qmd
```
