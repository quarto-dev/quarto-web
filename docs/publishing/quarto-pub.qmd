...
provider:
  id: quarto-pub
  name: Quarto Pub
  publish-url: "https://njones.quarto.pub/blog"
---
...
![](images/quarto-pub-admin.png){.border fig-alt="List of published sites on Quarto Pub, each shown as a card with a title and last updated date, with a sort-by menu above them."}
...
![](images/quarto-pub-admin-site.png){.border fig-alt="Site admin page with a Slug field, a Default Site checkbox, and a Delete Document button under Danger Zone."}
...
![](images/quarto-pub-default-site.png){.border fig-alt="Site admin page with the Default Site checkbox checked and the Save button below it."}
...
![](images/quarto-pub-tokens.png){.border fig-alt="Tokens panel showing an existing token with a Revoke button, and a New Token button above it."}
...
![](images/quarto-pub-new-token.png){.border fig-alt="Tokens panel with a new token name field and its generated token value shown above the existing token, with Save and Cancel buttons."}
...
2.  Add the Quarto Pub access token to your repository's action **Secrets** (accessible within repository **Settings**). You will see a **New repository secret** button at the top right:
    ...
    ![](images/gh-new-repository-secret.png){.border fig-alt="The GitHub repository's Settings page, on the Secrets and variables > Actions view, showing the New repository secret button in the Repository secrets section."}

    Click the button and add the personal access token from step 1 as a secret named `QUARTO_PUB_AUTH_TOKEN`:

    ![](images/gh-action-quarto-pub-secret.png){.border fig-alt="GitHub New secret form with the Name field set to QUARTO_PUB_AUTH_TOKEN and the personal access token pasted into the Value field."}
