...
If you are using the RStudio IDE, there is also support for push-button publishing to Posit Connect. Use the publish button <kbd>![](images/publish-button.png){width="23" height="20" fig-alt=""}</kbd> from the source editor or viewer pane to publish a document or a website:

![](images/rstudio-publish.png){.border fig-alt="Publish to RStudio Connect dialog with two options, Publish website with source code and Publish finished website only, the second of which is selected."}
...
![](images/import-from-git.png){fig-alt="Publish button menu open, with Import from Git listed above other options like R Markdown Document, Shiny App, and Jupyter Notebook."}
...
2.  Add the Connect API Key to your repository's action **Secrets** (accessible within repository **Settings**). You will see a **New repository secret** button at the top right:
    ...
    ![](images/gh-action-connect-secret.png){.border fig-alt="GitHub New secret form with the Name field set to CONNECT_API_KEY and the API key pasted into the Value field."}
