...
After authenticating, your content will be rendered and published, and then a browser opened to view its admin page on Connect. 
...
2.  Then, click the publish button <kbd>![](images/publish-button.png){width="23" height="20"}</kbd> from a notebook you wish to publish. You'll be prompted to configure a Connect server then be presented with a publishing dialog:
...
You can publish Quarto content to Connect using any CI service by scripting the `quarto publish` command. To do this, you'll need to make sure that your Connect server address and credentials are available as environment variables on your CI server
...
You will furthermore need to specify the ID of the target content to update. This is likely available in the `_publish.yml` file that is saved into your project directory during initial publish. For example:
...
Alternatively if you don't have a `_publish.yml`, you can specify the ID on the command line as follows:
...
Before creating your action, you need to ensure that your repository has the credentials required for publishing to Connect. To do this you need to do the following:
