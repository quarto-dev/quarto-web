...
Many users have reported problems with using `quarto publish gh-pages` on Windows systems. We are investigating and hoping to have a fix for this soon, but it the meantime we recommend against using `quarto publish gh-pages` on Windows.
:::
