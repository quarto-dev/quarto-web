...
`quarto publish gh-pages` automatically creates a `gh-pages` branch (locally and on the remote) on its first run if one does not already exist. In most cases you do not need to create or configure this branch yourself.

For **project sites** (`https://<username>.github.io/<repo>/`), GitHub detects the new `gh-pages` branch and configures GitHub Pages to deploy from it automatically --- no manual configuration under **Settings** : **Pages** is required.

For **user or organization sites** (`https://<username>.github.io/`), the GitHub Pages source branch defaults to `main`. After the first publish you need to switch the source branch to `gh-pages` under **Settings** : **Pages** for your repository:
