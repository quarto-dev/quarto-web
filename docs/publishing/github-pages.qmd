...
[GitHub Pages](https://pages.github.com/) is a web publishing service that enables you to publish content based on source code managed within a GitHub repository.
...
2.  Use a [GitHub Action] to automatically render and publish content whenever you push a source code change to your repository.
...
It's important to note that you don't need to check your `_site` or `_book` directory into version control (if you have done this in the past you know it makes for very messy diffs!). Before proceeding you should add the output directory for your project to `.gitignore`. For example:
...
If you have executable code in your project you'll notice that a `_freeze` directory has been created at the top level of your project. This stores the results of computations and should be checked in to version control. Whenever you change a `.qmd` file with executable code, it will automatically be re-run during your next render and the updated computations stored in `_freeze`.

Note that below we'll cover [Executing Code] within a GitHub Action but for now we'll keep things simpler by using freeze.
