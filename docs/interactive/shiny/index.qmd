...
In order to run the examples below you will need the development version of the **rmarkdown** package, which you can install with:
...
2.  The YAML front-matter includes the `echo: false` option (to indicate that we don't want R code chunks to print their source code by default).
3.  We add `classes: panel-sidebar` and `classes: panel-fill` to the first two R code chunks to specify that we want them layed out in special panel types.
4.  We again use `context: server` on the last R code chunk to indicate that it contains the Shiny Server code.
...
See the article on [Page Layout](../layout.qmd) to learn more about the available tools for managing the layout of interactive documents.
...
-   [JavaScript & Shiny](ojs.qmd) explains how to use Shiny to dynamically transform data for use in interactive graphics created with [Observable JavaScript](../js/).

-   [Page Layout](../layout.qmd) enumerates the various techniques you can use to layout interactive components within your documents.
