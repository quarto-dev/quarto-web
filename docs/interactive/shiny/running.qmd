...
2.  Use the `quarto run` command line interface.
3.  Deploy them to a server for use by a wider audience.

We'll cover all of these scenario in depth here. Note that in order to run interactive Shiny documents you will need the development version of the **rmarkdown** package which you can install as follows:
...
Note that you need the [daily build](https://dailies.rstudio.com) of RStudio in order to run Quarto interactive documents, which you can download here <https://dailies.rstudio.com>.
...
2.  The very latest development versions of the **rsconnect** and **quarto** R packages. You can install them as follows:

        remotes::install_github("rstudio/rsconnect")
        renotes::install_github("quarto-dev/quarto-r")
