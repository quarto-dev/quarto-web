...
We'll explore all of these techniques below, and also touch briefly on creating a live connection between OJS reactives and Shiny reactives.
...
```{ojs}
data = {
  const co2data = await FileAttachment("co2_mm.csv").csv();
...
This example used the `csv()` method of `FileAttachment`. A similar method is available for each supported format:
...
But what if you want to do data transformation dynamically in response to user inputs? This is also possible with `ojs_define()`, as it can be passed not just static values but also Shiny reactives (assuming it's running inside a Shiny [interactive document](../shiny/ojs.qmd)).
...
<a href="https://jjallaire.shinyapps.io/kmeans-shiny-ojs/"><img src="images/kmeans-shiny-ojs.png" class="img-fluid border"></a>
...
Note that the plotting code references the variables `selectedData` and `clusters`. These will be provided by reactive expressions within the Shiny server code. Note also that we use the `transpose()` function to reshape the data into the row-oriented format that plot expects.
...
Learn more about using OJS and Shiny together in the article on [JavaScript and Shiny](../shiny/ojs.qmd).
