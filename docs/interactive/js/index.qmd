...
The creators of Observable run a hosted service at <https://observablehq.com/> where you can create and publish notebooks. Additionally, you can use Observable in standalone documents and websites via its [core libraries](https://github.com/observablehq). Quarto uses these libraries along with a [compiler](https://github.com/asg017/unofficial-observablehq-compiler/tree/beta) that is run at render time to enable the use of Observable JavaScript ("OJS") within Quarto documents.
...
That covers a basic end-to-end use of OJS (see the [Penguins](examples/penguins.qmd) examples for the full source code).

::: {.callout-tip appearance="simple"}
If you take a look at the [Penguins](examples/penguins.qmd) code, you'll notice something curious: the inputs and plotting code are defined *before* the data processing code. This demonstrates a critical difference between OJS cell execution and traditional notebooks: cells do not need to be defined in any particular order.
...
In our initial example we used a [FileAttachment](https://github.com/observablehq/stdlib#file-attachments) as our data source. File attachments support many formats including CSV, TSV, JSON, SQLite, and Arrow so are a convenient way to read a dataset that has already been prepared for analysis.
...
```{python}
import pandas as pd
penguins = pd.read_csv("examples/palmer-penguins.csv")
...
See the article on [Data Sources](data-sources.qmd) to learn more about the various ways to prepare and read data.
...
To learn about all of the options available, see the article on [OJS Cells](ojs-cells.qmd).

## Learning More

These articles go into more depth on using OJS in Quarto documents:

-   [Libraries](libraries.qmd) covers using standard libraries and external JavaScript libraries.

-   [Data Sources](data-source.qmd) outlines the various ways to read and pre-process data.

-   [OJS Cells](ojs-cells.qmd) goes into more depth on cell execution, output, and layout.

-   [Shiny Reactives](shiny.qmd) describes how to integrate Shiny with OJS.

-   [Code Reuse](code-reuse.qmd) delves into ways to re-use OJS code across multiple documents.
