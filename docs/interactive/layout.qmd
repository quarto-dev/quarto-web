...
![](images/article-input-panel-columns.png){fig-alt="An input panel with three inputs in three columns: Passport color checkboxes, Representation radio buttons, and a Symbol size slider. A world map of passport colors is below the panel."}
...
![](shiny/images/iris-k-means.png){.border fig-alt="Shiny app titled Iris K-Means Clustering that fills the page width. A sidebar on the left has X Variable, Y Variable, and Cluster count inputs. A scatterplot of the clusters is on the right."}
...
![](images/application-sidebar-panel.png){fig-alt="A sidebar panel on the left with a dropdown, a slider, and two sets of radio buttons. A map of vaccination rates in France fills the main area to the right."}
...
![](images/application-panel-layout.png){.border fig-alt="A panel layout with a Height and Weight heatmap across the full first row, and two charts, Average Height and Average Weight, side by side in the second row."}
