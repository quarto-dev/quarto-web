...
[![](images/kmeans-shiny-ojs.png){.border fig-alt="K-Means Clustering app with X Variable, Y Variable, and Cluster Count inputs in a sidebar, and a scatterplot of three colored clusters of iris data. Below are OJS Client and Shiny Server tabs. The Shiny Server tab shows R code that ends with ojs_define(selectedData, clusters)."}](https://jjallaire.shinyapps.io/kmeans-shiny-ojs/)
