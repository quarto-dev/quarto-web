...
In R Markdown documents a `setup` chunk is often used to establish document-wide Knitr options. You can still do this in Quarto, but you can also specify Knitr options in YAML. For example:
...
Here we establish default Knitr options for a single document. You can also add shared `knitr` options to a project-wide `_quarto.yml` file or a project-directory based `_metadata.yml` file.
