...
::: callout-caution
When using the Knitr engine, `fig-width` and `fig-height` are supported on a per-cell basis. But when using the Jupyter engine, 
these options only have an effect if specified at the document- or project-level metadata.
:::
...
::: {.callout-note}
# Bash and Python coexistence
Bash and Python code cells can coexist as you have them here only in Knitr engine (add `engine: knitr` in front matter). If on the other hand you use [Jupyter shell magics](https://jakevdp.github.io/PythonDataScienceHandbook/01.05-ipython-and-shell-commands.html), you can do something similar in Jupyter (but not if this is quite as general).
:::
