...
2.  It can serve as the basis for a format template (which can help users gets started using your format more easily).
...
author: "Your Name"
date: today
format: lexcorp-pres-revealjs
---

# Overview
```

Extension repositories and structured in such a way that you can test your extension and the template by simply rendering the `template.qmd` file right in the root of your repository. The `template.qmd` will be able to load your extension just as it would when installed, so testing and iterating should be as simple as working within your extension directory until you're satisfied (without the need to repeatedly install or update the extension in order to test it).

## Distributing Formats

You can distribute format extensions in one of two ways:

1.  As a template that includes both the format in the `_extension` directory and the `template.qmd` (which is automatically renamed to match the name of the enclosing directory).
2.  As a plain format with no template scaffolding (this is useful for adding the format to an existing document or project).

If you have a GitHub repository containing the files enumerated above in the `lexcorp-pres` example, users could install your extension and associated template as follows (where `lexcorp` is the GitHub organization hosting the repo):

``` bash
quarto use template lexcorp/lexcorp-pres
```

This is often the preferred way to get started with a format as it provides the user with a working document right out of the box. It's also possible to just install the format if you working with an existing project:

``` bash
quarto install extension lexcorp/lexcorp-pres
```
...
In some cases format extensions will want to make use of other extensions. This is permitted, but installing extensions for use within a custom format must be done with a special syntax to ensure they are embedded correctly.
...
This will install the `quarto-ext/fancy-text` extension into the `quarto-journals/jss` extension in the `_extensions` folder. By embedding an extension you make it available without creating the potential for conflict with other versions of the extension that might be in use in the current project.
