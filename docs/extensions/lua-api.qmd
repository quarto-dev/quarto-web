...
| `quarto.doc.add_resource(path)`                 | Add a resource file to the current render, copying that file to the same relative location in the output directory. Learn more about resource handling in [Site Resources](/docs/websites/website-tools.qmd#site-resources).                                                                                                                                       | 
| `quarto.doc.add_supporting(path)`               | Add a supporting file to the current render, moving that file to the same relative location in the output directory. Supporting files can be cleaned up after rendering.                                                                                                                                 |
...
Quarto defines custom AST node types that you can create programmatically in Lua filters. For details on the node types themselves (fields available when processing nodes in filters), see [Quarto AST: Custom Nodes](/docs/advanced/quarto-ast.qmd#custom-nodes).
...
- - `quarto.Callout(tbl)`
  - Creates a [Callout](/docs/advanced/quarto-ast.qmd#callouts).
  - Required: `type`

    Optional: `title`, `content`, `appearance`, `collapse`, `icon`, `attr`
  - `title` and `content` accept strings. `attr` sets div attributes.

- - `quarto.Tabset(tbl)`
  - Creates a [Tabset](/docs/advanced/quarto-ast.qmd#tabsets).
  - Optional: `tabs`, `level`, `attr`
  - Use `quarto.Tab()` to create entries for `tabs`.

- - `quarto.Tab(tbl)`
  - Creates a Tab for use in a Tabset.
  - Required: `title`

    Optional: `content`, `active`
  - `title` and `content` accept strings.

- - `quarto.ConditionalBlock(tbl)`
  - Creates a [Conditional Block](/docs/advanced/quarto-ast.qmd#conditional-blocks).
  - Required: `node`, `behavior`

    Optional: `condition`
  -

- - `quarto.FloatRefTarget(tbl)`
  - Creates a [cross-referenceable float](/docs/advanced/quarto-ast.qmd#cross-referenceable-elements).
  - Required: `type`, `content`, `identifier`

    Optional: `caption_long`, `caption_short`, `classes`, `attributes`, `attr`, `parent_id`
  - `attr` is an alternative to separate `identifier`, `classes`, `attributes` fields.
:::
