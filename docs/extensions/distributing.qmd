...
You can also install an extension from a subdirectory of a GitHub repository. For example, here we install two different extensions from the `cooltools/icons` repository:

``` {.bash filename="Terminal"}
quarto install extension cooltools/icons/fontawesome
quarto install extension cooltools/icons/iconify
```
