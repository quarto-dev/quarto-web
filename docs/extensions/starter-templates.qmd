...
2.  Provide the intiial content for a custom [Project Type](project-types.qmd). 
...
1. It provides an easy to test that your template is working as expected.
...
When creating [Journal Articles](../journals/index.qmd), [Custom Formats](formats.qmd), or [Project Type](project-types.qmd) extensions, we recommend that you additionally provide a starter template to make it easy for user to get started.

This is generally is as easy as adding a `template.qmd` file to your extension that demonstrates its use. With this configuration, users can either begin using your extension via the template or by a conventional `quarto install` of the extension. 
...
Alternatively, you can the format (without the template)into an existing project or directory:
