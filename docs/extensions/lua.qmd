...
1. Develop filters and extensiosn using the [Quarto v1.2 Pre-Release](https://quarto.org/docs/download/prerelease.html), which includes support for live reload of `quarto preview` whenever a Lua source file changes.

2. Use the `quarto.utils.dump()` function to inspect the Pandoc AST elements that you are producing and consuming. For example:
...
3. Use the `print()` function to print debugging output to standard output (you'll see it in the `quarto preview` terminal output).

4. It can sometimes be helpful to change the output format of your document to `native` --- when you do this you'll see the full Pandoc AST rather than a rendering to HTML, PDF, etc. 
