---
title: Author and Affiliation Processing
draft: true
---
...
This is implemented as a (relatively) [standalone LUA filter in Quarto](https://github.com/quarto-dev/quarto-cli/tree/main/src/resources/filters/authors).
...
The normalized authors schema at the top level is a closed schema. Unrecognized keys that are passed in the root of authors will be normalized under the metadata key. Keys that are normalized into `metadata` should be considered potentially template specific and may not be present or depended upon when implementing a template.
