...
Currently, brand extensions merge project-level metadata, so they can only be used with a [`_quarto.yml` project](../projects/quarto-projects.qmd) present where they are installed. This limitation may be lifted in the future.
...
As shown above, you'll be prompted for an extension name. Type the name of your brand extension and press Enter---the brand extension is then created. We'll use `my-brand` in these docs.
...
The example `brand.yml` is using the new _unified light and dark brand_ syntax. Documentation for this feature is on the way.
...
This works only for the color properties. There is a similar syntax for logos.
...
Any assets, such as logos or fonts, will be resolved relative to the brand file.

This way, when the extension is installed in a Quarto project, all paths are finally resolved relative to the project.

The brand is resolved via the `project.brand` key, which takes a path to a unified light and dark brand.yml file. Because it is resolved through the `project` key, brand extensions require a `_quarto.yml` project where they are installed. The project can be any type. 
