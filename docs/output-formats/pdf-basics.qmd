...
Note the use of the `|` character on the line with `text:` inside `include-in-header:` to indicate that the value is a multi-line string.

You might wish to be explicit about the type of include you're using when using multiple declarations. For that use case, `include-in-header` also supports the `file:` key:
...
format:
  pdf:
    include-in-header:
      ...
      - file: packages.tex
      - macros.tex 
      # if you omit `file:` or `text:` in 
      # `include-in-header`, quarto assumes you meant a file
```
