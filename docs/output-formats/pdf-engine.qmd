...
Note that as of [Quarto v1.2](https://quarto.org/docs/download/prerelease.html), TinyTeX is not installed to the system `PATH` so will not affect other applications that use TeX. If you want to use TinyTeX with other applications, add the `--update-path` flag when installing (this will add TinyTex to the system path):

```{.bash filename="Terminal"}
quarto install tool tinytex --update-path
```

If you are using Quarto v1.2 and already have another installation of TinyTeX that you prefer to use with Quarto, add the `latex-tinytex: false` in your project or document front matter to prevent Quarto from using its internal version.
