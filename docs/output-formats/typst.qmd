...
The `fancy` appearance integrates with Quarto's [brand system](/docs/authoring/brand.qmd), using your brand colors for theorem boxes.
...
Quarto passes alt text from markdown image captions and `fig-alt` attributes to Typst's `image()` function, making images accessible to screen readers. See [Figures](/docs/authoring/figures.qmd) for details on specifying alt text.
