...
::: {.list-table widths="0.15697674418605,0.84302325581395" header-rows="1"}
...
* * `location`
  * Location on the page in `X-Y`, where `X` is `left`, `center`, or `right` and Y is `top`, `middle`, or `bottom`.
...
* * `padding`, `padding-top`,
    ...
    `padding-bottom`,
    `padding-left`
  * Amount of padding to add to each side of the logo, using CSS [`padding`](https://developer.mozilla.org/en-US/docs/Web/CSS/padding) conventions.
    Padding options are applied in alphabetical order.
    Applied as Typst [`block.inset`](https://typst.app/docs/reference/layout/block/#parameters-inset).
    Default `padding: 0.75in`.
