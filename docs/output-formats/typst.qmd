...
# project:
#   pre-render:
#     - quarto run ../../_tools/snapshot-typst.ts examples/gt-temps.qmd images/typst-tables-gt-temps.png
---
...
Typst is also supported for [book projects](/docs/books/book-output.qmd#typst-output).
...
Quarto uses the [theorion](https://typst.app/universe/package/theorion) package for theorems in Typst output. You can control the appearance of theorems, lemmas, proofs, and related blocks using the `theorem-appearance` option:
...
The `fancy` appearance integrates with [`_brand.yml`](/docs/authoring/brand.qmd), using your brand colors for theorem boxes.
...
#### R - gt
:::: {layout-ncol=2}
![](images/typst-tables-gt-temps.png){height=200px fig-alt="Table of January and June temperatures for 1920 to 1924, with each cell shaded from green for cooler values to orange for warmer values."}
...
#### Python - Pandas
:::: {layout-ncol=2}
![](images/typst-tables-pandas-temps.png){height=200px fig-alt="Table of January and June temperatures for 1920 to 1924, with each cell shaded from green for cooler values to orange for warmer values."}
...
::: callout-tip
Typst also validates documents against the requested standard and will fail to produce a PDF if it detects non-compliance. Its validation is thorough but not as complete as veraPDF.
:::

### Image Alt Text

Quarto passes alt text from markdown image `fig-alt` attributes to the `alt` parameter of Typst's `image()` function, making images accessible to screen readers. See [Figures](/docs/authoring/figures.qmd#alt-text) for details on specifying alt text.

### Equation Alt Text {#equation-alt-text}

Equations in Typst can include alternative text for screen readers.
Alt text is supported for cross-referenced equations using the `alt` attribute in the equation label:
...
::: {.callout-warning}
## No alt text on inline equations
It is not possible under Pandoc syntax to provide alt text for inline equations. Use a cross-referenced block/display equation for PDF accessibility in Typst.
:::
