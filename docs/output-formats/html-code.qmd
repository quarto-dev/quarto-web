...
Code linking is currently implemented only for the knitr engine (via the [downlit](https://downlit.r-lib.org) package) and does not work with `code-line-numbers: true`. 