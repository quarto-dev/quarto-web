...
::: {.callout-note}
Code linking will not work when `code-link` and `code-line-numbers` are both set to `true` due to a current [downlit limitation](https://github.com/r-lib/downlit/issues/133).
You can follow [quarto-dev/quartoc-cli#2036](https://github.com/quarto-dev/quarto-cli/issues/2036) for resolution.
:::