...
The `code-line-numbers` option enables the display of line numbers in code blocks:
...
## Insert Quarto executable source code, without executing it

If you want to insert it, you must escape it using four backtick symbols (`` ```` ``), and add another pair of curly brackets around the language statement (in example from `{python}` to `{{python}}`).

In example if you will write
...
you will have the text below, without executing it
