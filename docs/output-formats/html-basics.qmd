...
You may also enable [Beblob](https://gitlab.com/antonbelev/beblob) for commenting using the `beblob` option. Unlike Utterances and Giscus, which are backed by GitHub, Beblob is backed by **GitLab**: it stores comments as issues in a GitLab project, and works with both gitlab.com and self-managed GitLab instances.

First [register a GitLab OAuth application](https://docs.gitlab.com/integration/oauth_provider/) (on gitlab.com or your self-managed instance) with the `api` scope and a redirect URI pointing at your published site. Then provide at least the application's `client-id`, the matching `redirect-uri`, and the `project-name`:
...
`project-name` is the GitLab project's *name* as shown in GitLab (Beblob looks the project up by searching for this name), **not** its `group/project` path. For a self-managed GitLab instance, set `gitlab-url` to its base URL:
...
You can also control how pages map to issues with `issue-mapping-strategy` and the appearance with `theme`. These and other options are [covered here](/docs/reference/projects/websites.qmd#beblob).
