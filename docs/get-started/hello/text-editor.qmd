...
You can get reasonable syntax highlighting for Quarto source files in any editor by mapping the `.qmd` file extension to the "markdown" file type.
This will handle markdown well, but executable code blocks may not be highlighted in their native language.
This is because Quarto uses a special syntax for executable code (e.g. ```` ```{python} ```` rather than ```` ```python ````).

There are Quarto syntax highlighting modes available for several editors that provide additional support for Pandoc markdown and handle executable code blocks correctly.
You may want to install and configure one of these before proceeding with the tutorial.
...
+--------------+---------------------------------------------------------------------+
| VS Code      | <https://marketplace.visualstudio.com/items?itemName=quarto.quarto> |
+--------------+---------------------------------------------------------------------+
...
![](images/qmd-how-it-works.png){fig-align="left"}
...
Then, save the file.
You'll see that the document is re-rendered, and the browser preview is updated.
