...
Let's create a notebook and add some formats and options to it.
...
So for the options above, if we executed this:
...
Do this by adding the `toc` and/or `number-sections` options to document options (note that these options are typically specified at the root level because they are shared across all formats):
...
To create a display equation use `$$…$$`.
...
Cross-reference-able entities generally requires a label (unique identifier) and caption.
...
Here's how we expressed each of these:
