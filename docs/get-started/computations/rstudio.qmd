...
![](images/rstudio-exec-echo-true-preview.png){.border .column-page-right}
...
Let's add another plot to our chunk: another scatterplot where the points are colored by engine displacement, using a different color scale.
...
![](images/rstudio-multi-figure-preview.png){.border}
...
See the article [Figure Layout](https://tutorial.quarto.org/docs/authoring/figure-layout.html) for details.
...
See the documentation on [Article Layout](https://tutorial.quarto.org/docs/authoring/article-layout.html) to learn about all of the available layout options.
...
``` markdown
There are _r nrow(mpg)_ observations in our data. 
```
...
``` markdown
The average city mileage of the cars in our data is _r mean_cty_ and the average highway mileage is _r mean_hwy_.
```
...
::: {.callout-note appearance="minimal"}
<i class="bi bi-journal-code"></i> [Download computations-complete.qmd](_computations-complete.qmd){download="computations-complete.qmd"}
:::
