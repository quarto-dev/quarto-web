...
Switch to this directory in a terminal, install tutorial dependencies (if required), then open Jupyter Lab so we can start working with the notebook:
...
There are lots of other options available for cell output.
...
Rather than hiding code entirely, you might want to fold it and allow readers to show it at their discretion .
...
Copy and paste the below into the notebook if you want to try these changes locally:
...
If you have 3, 4, or more figures in a panel there are lots of options available for customizing their layout.
