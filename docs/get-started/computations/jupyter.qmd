...
In addition to code cells, you can also include code inline in markdown cells in your notebook.
...
Try adding the following to a new markdown cell in your notebook.

````markdown
There are `{python} len(gapminder)` rows in the `gapminder` data.
````

Inline expressions won't be evaluated within JupyterLab. To see the result, render the notebook with Quarto:
