...
This will search for `Agbalumo` font also in a local `./myfonts/` directory, for a `.ttf` or `.otf` matching the font name.

`TYPST_FONT_PATHS` environment variable is also taken into account for compatibility with Typst configuration, and setting `font-paths` will take precedence over any path set in TYPST_FONT_PATHS environment variable.
