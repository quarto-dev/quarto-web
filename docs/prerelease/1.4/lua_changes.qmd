...
---
filters:
  - entry-point: before-ast
    path: filter1.lua
  - entry-point: after-ast
    path: filter2.lua
  - entry-point: before-quarto
    path: filter3.lua
  - entry-point: after-quarto
    path: filter4.lua
  - entry-point: before-render
    path: filter5.lua
  - entry-point: after-render
    path: filter6.lua
  - pre.lua
  - quarto
  - post.lua
...
The user-accessible entry points for user filters are: `before-ast`, `after-ast`, `before-quarto`, `after-quarto`, `before-render`, `after-render`. They appear before and after each of the three main stages above.
The `before-ast` and `before-render` entry points are new. 
In 1.4, `before-quarto` and `after-ast` point to the same location in
the filter chain (similarly for `before-render` and `after-quarto`). 
...
"Pre" filters are injected at the `before-quarto` entry point, and "post" are injected at the `after-render` entry point.
