...
1. The `context: setup` cell options indicates that this code cell should run when the application starts (as opposed to when each new client session starts). Expensive initialization code (e.g. loading data) should be placed in `context: setup`.
...
3. These checkbox input groups have their contents dynamically driven from the available categories in the `species` and `islands` fields.

4. When the user interacts with the checkbox groups this results in a different filtered view of the dataset The `@reactive.Calc` function here recomputes the filtered dataset and makes it available as `filtered_penguins()`.
...
7. The `layout` cell options defines how multiple outputs (e.g. plots) within a call should be position. Here we specify that there should be 2 rows, with the top row having a single plot and the second row equally divided between the other two plots.
...
9. The Data tab also references `filtered_penguins()` and is updated whenever the filter changes.
