...
That said, if you do wish to tailor execution in a more fine grained way, the following additional contexts are available:

| Context                 | Description                                                                                                                     |
|-------------------------|---------------------------------------------------------------------------------------------------------------------------------|
| `context: ui`           | Cell will only run during `quarto render`. Code and data created by this cell are not available when the application is served. |
| `context: server`       | Cell will run for each new client session. Will not run during `quarto render`. Output of cell is not included in HTML UI.      |
| `context: server-setup` | Cell will only run at server startup. Will not run during `quarto render`. Output of cell is not included in HTML UI.           |
: {tbl-colwidths="[40,60]"}

Note that `context: setup` is simply a combination of `context: ui` and `context: server-setup`.
