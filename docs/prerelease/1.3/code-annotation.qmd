...
This example, rendered in HTML and PDF formats, uses annotation to draw attention to the first line of code and the while loop: 
...
In PDF and HTML formats, annotations are numbered using an annotation marker displayed in the code. In other formats, rather than numbering annotations, annotations are labelled with the line of code (or lines of code) to which the annotation text applies.

Annotations are added using code comments combined with an ordered list using a special syntax. The `code-annotations` option controls whether annotation is disabled (`false`), if annotations should be removed from the output (`none`), and for the HTML format, how annotations should appear (`below` (default), `hover` or `select`).
...
Here's an example that uses `code-annotations: select` rather than the default `code-annotations: below`:
...
For some formats, you may prefer to remove annotations from the output. In this case, you can set `code-annotations: none`, which will remove the annotation comments for your code and suppress the output of the ordered list which contains the annotation text.
