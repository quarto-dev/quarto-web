...
To publish a collection of documents, organise your documents in a [Quarto project](/docs/projects/quarto-projects.html), using the `confluence` project type. Here's a minimal `_quarto.yml` file for a Confluence project:
...
You'll be walked through the same steps as publishing a single document, [setting up an account](#setting-up-account), if needed, and [selecting a destination](#selecting-destination) for your project on Confluence, finishing with your project being published to Confluence.

### Project Structure {#project-structure}

The hierarchy of documents inside folders in your project will be respected in the publishing process. However, since Confluence has no concept of folders, folders are represented by pages in Confluence.
...
![](images/confluence-project-structure.png){fig-alt="A zoomed in view of the navigation sidebar in Confluence. Under Pages is a page called Project Example Site, nested under this page are pages called Team Members, Project Roadmap, and Reports-folder. Under the Reports-folder page are pages called: Reports, March, and January."}

The titles used in the Confluence sidebar navigation are taken from the page and project title, as specified in the document YAML and `_quarto.yml` respectively, and generated from the folder name for folders.
...
To help avoid a situation where someone inadvertently edits a page being managed in Quarto, the permissions for pages are set when you publish so everyone can view the page, but only you, the publisher, can edit the page.
...
::: callout-note
## Free Confluence Accounts

Free Confluence accounts do not include control over page permissions. Any page you publish can be both viewed and edited by anyone.
:::
