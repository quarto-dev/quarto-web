...
1.  **Cell `id`**\
    First, the cell metadata will be checked for a matching `id` , if found, this cell will be embedded. (Cell ids are a newer feature of Jupyter Notebooks that are not yet well supported in Jupyter front ends, but this is checked for future compatibility as ids become more used.)
2.  **Label**\
...
For example, to embed the output of a cell that has a specific tag, like the following (which is tagged with `plot-line`):
...
embeds both the code for the cell as well as the outputs of the code cell:
