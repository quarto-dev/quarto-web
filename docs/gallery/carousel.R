...
carousel <- function(id, duration, items) {
  ...
  items <- lapply(items, function(item) {
    index <<- index + 1
    carouselItem(item$caption, item$image, item$link, item$alt, index, duration)
  })
...
# carousel item
carouselItem <- function(caption, image, link, alt, index, interval) {
  ...
  item <- div(class = paste0("carousel-item", ifelse(index == 0, " active", "")),
              ...
              a(href = link, img(src = image, alt = alt, class = "d-block  mx-auto border")),
